...
class TakePhoto extends Component {
    ...
    if (hasCameraPermissions === null) {
      ...
      return (
        <View style={styles.container}>
          ...
          {pictureTaken ? (
            <View style={{ flex: 2 }}>
              <FitImage source={picture} style={{ flex: 1 }} />
            </View>
          ) : (
            <Camera
              type={type}
  ...
  _takePhoto = async () => {
    ...
    if (!pictureTaken) {
      if (this.camera) {
        ...
        this.setState({ picture: takenPhoto, pictureTaken: true });
      }
    }
  };
  _rejectPhoto = () => {
  ...
  _approvePhoto = async () => {
    ...
    const saveResult = await CameraRoll.saveToCameraRoll(picture.uri, "photo");

    changeProfileImage(picture);
    changeMain();
    /*
    this.setState({
      picture: null,
